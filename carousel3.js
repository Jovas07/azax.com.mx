var carousel = (function(){
    ...
    var container = document.getElementById('carousel4'),
    ...
    function complete() {
        if ( animating ) {
            ...
            container.classList.remove( 'animate3' );
            // change the ordinal
            changeOrdinal();
    ...
    function changeOrdinal() {
        ...
        var index = active-1;
        /* if the active item was 0, we're now at -1 so set to the last item */
        if (index < 0) {
            index = length-1;
        }
        ...
        while ( ordinal < length ) {
            ...
            /* as we are working from active we need to go back to the start if we reach the end of the item list */
            if ( index < length-1 ) {
                index++;
            } else {
                index = 0;
    ...
    return {
        init: function() {
            var navigation = document.querySelectorAll( 'a.navigation3' );
